...
function novoUsuario(email, senha) {
  firebase
    ...
    .then((result) => {
      ...
      window.location.href = `${urlApp}/funcionarios.html`; 
    })
    .catch((error) => {
      console.error(error.code);
      console.error(error.message);
      alerta(
        `Erro: Não foi possível cadastrar o usuário <br> ${errors[error.code]}`,
        "danger"
      );
    });
}
...
function logoutFirebase() {
  firebase
    ...
    .catch(function (error) {
      alerta(
        `Erro: Não foi possível efetuar o logout <br> ${errors[error.code]}`,
        "danger"
      );
    });
}
